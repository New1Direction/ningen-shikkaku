...
mod imp {
    ...
    pub fn apply() -> Result<()> {
        ...
        for name in RUNTIME_REQUIRED {
            match ScmpSyscall::from_name(name) {
                Ok(syscall) => match filter.add_rule(ScmpAction::Allow, syscall) {
                    Ok(()) => runtime_added += 1,
                    Err(e) => eprintln!("[dazai-seccomp] note: could not allow {name}: {e}"),
                },
        ...
        eprintln!(
            "[dazai-seccomp] seccomp active: {} core + {} runtime syscalls allowed; default = KillProcess",
...
mod imp {
    ...
    pub fn apply() -> Result<()> {
        eprintln!(
            "[dazai-seccomp] WARNING: seccomp NOT active (requires Linux + the `seccomp` feature). \
